...
#include <errno.h>// this is used to access the special variable errno which details information about the specific error
...
int my_function(char *cmd)
{
	...
	else if(pid==0)
	{
			...
			char *argv[]={"vim","/home/georgeca/myJoke.txt",NULL};

	        int ret=execv("/usr/bin/vim",argv);
